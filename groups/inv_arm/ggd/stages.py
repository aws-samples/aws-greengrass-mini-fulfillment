...
if __name__ == '__main__':
    ...
    with ServoProtocol() as sp:
        sg = ServoGroup()
        sg['base'] = Servo(sp, config.arm_servo_ids[0])
        sg['femur01'] = Servo(sp, config.arm_servo_ids[1])
        sg['femur02'] = Servo(sp, config.arm_servo_ids[2])
        sg['tibia'] = Servo(sp, config.arm_servo_ids[3])
        sg['effector'] = Servo(sp, config.arm_servo_ids[4])
